...
namespace RdKafka
{
    public partial class Producer
    {
        ...
        public class Topic : IDisposable
        {
            ...
            public Task<RecordMetadata> Produce(byte[] payload, byte[] key = null, Int32 partition = RD_KAFKA_PARTITION_UA)
            {
                // Passes the TaskCompletionSource to the delivery report callback
                // via the msg_opaque pointer
                var deliveryCompletionSource = new TaskCompletionSource<RecordMetadata>();
